...
public class addbookController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        // automatically generate an ID when the window opens
        generateAndSetID();
        bookIDField.setEditable(false); // not changeable
        bookIDField.setStyle("-fx-background-color: #e0e0e0;");
    }
    ...
    private void generateAndSetID() {
        ...
        while (true) {
            int num = random.nextInt(901) + 100; // Generates 100 to 1000
            newID = "#" + num;
            if (!db.isBookIdExists(newID)) {
                break;
            }
        }
